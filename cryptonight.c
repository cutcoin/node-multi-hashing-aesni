...
void cryptonight_hash(const char* input, char* output, uint32_t len) {
  ...
  __m128i a_x = _mm_load_si128((__m128i *)ctx->a);
	__m128i b_x = _mm_load_si128((__m128i *)ctx->b);

	for(int i = 0; __builtin_expect(i < 0x80000, 1); i++)
	{
    uint64_t off = _mm_cvtsi128_si64(a_x) & 0x1FFFC0;
    __m128i c_x = _mm_load_si128((__m128i *)&ctx->long_state[off]);
    c_x = rotate(c_x, b_x, 0);
    for (int j = 1; j < 4; j++) {
      c_x = _mm_add_epi32(c_x, _mm_load_si128((__m128i *)&ctx->long_state[off + 16 * j]));
      c_x = rotate(c_x, b_x, j);
    }

    c_x = _mm_aesenc_si128(c_x, a_x);

    for (int j = 0; j < 4; j++) {
      __m128i tmp = rotate(b_x, a_x, j);
      tmp = _mm_add_epi32(tmp, c_x);
      tmp = _mm_xor_si128(tmp, _mm_load_si128((__m128i *)&ctx->long_state[off + 16 * j]));
      _mm_store_si128((__m128i *)&ctx->long_state[off + 16 * j], tmp);
    }

    off = _mm_cvtsi128_si64(c_x) & 0x1FFFC0;

    __m128i tmp = _mm_load_si128((__m128i *)&ctx->long_state[off]);
    tmp = rotate(tmp, a_x, 0);
    for (int j = 1; j < 4; j++) {
      tmp = _mm_sub_epi32(tmp, _mm_load_si128((__m128i *)&ctx->long_state[off + 16 * j]));
      tmp = rotate(tmp, a_x, j);
    }
    ...
    for(int j = 0; j < 4; j++)
    {
      c_x = rotate(a_x, tmp, j);
      c_x = _mm_xor_si128(c_x, _mm_load_si128((__m128i *)&ctx->long_state[off + 16 * j]));
      _mm_store_si128((__m128i *)&ctx->long_state[off + 16 * j], c_x);
    }
